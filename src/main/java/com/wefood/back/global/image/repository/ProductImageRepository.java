...
public interface ProductImageRepository extends JpaRepository<ProductImage, ProductImage.Pk> {
    ...
    @Query("select new com.wefood.back.product.dto.ProductImageDetailResponse(i.name, pi.isThumbnail, pi.sequence) from Image i inner join ProductImage pi on i.id=pi.pk.imageId where pi.pk.productId=:productId")
